...
export default function SupplyRow({ token, Withdraw, Supply }) {
  const userTokenLentAmount = token.userTokenLentAmount || {};
  const amount = userTokenLentAmount.amount || 0;
  const inDollars = userTokenLentAmount.inDollars || 0;
    ...
    <>
      <tr>
        ...
        <td className="border-t-0 px-4 border align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
          <div className="text-base font-medium text-gray-800">{todp(amount, 3)}</div>
