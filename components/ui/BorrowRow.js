...
export default function BorrowRow({ token, Repay, Borrow }) {
  const userTokenBorrowedAmount = token.userTokenBorrowedAmount || {};
  const amount = userTokenBorrowedAmount.amount || 0;
  const inDollars = userTokenBorrowedAmount.inDollars || 0;
    ...
    <>
      <tr>
        ...
        <td className="border-t-0 px-4 border align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
          <div className="text-base font-medium text-gray-800">{todp(amount, 4)}</div>
          <div className="text-sm text-gray-500">${todp(inDollars, 2)}</div>
        </td>
        <td className="border-t-0 px-4 border align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
